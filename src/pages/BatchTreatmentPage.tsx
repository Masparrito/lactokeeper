...
const TreatmentCard = ({ event }: { event: Event }) => {
    return (
        <div className="bg-c-surface backdrop-blur-xl rounded-2xl p-4 border border-c-border">
            <p className="font-semibold text-c-text">{event.details}</p>
        ...
        <div className="w-full max-w-2xl mx-auto space-y-6 pb-12 animate-fade-in">
            <header className="flex items-center pt-8 pb-4">
                ...
                <div className="text-center flex-grow">
                    <h1 className="text-3xl font-bold tracking-tight text-c-text">Tratamientos: {lotName}</h1>
                    <p className="text-lg text-c-text-muted">Sanidad Colectiva del Lote</p>
                </div>
            ...
            <form onSubmit={handleSubmit} className="bg-c-surface backdrop-blur-xl rounded-2xl p-4 border border-c-border space-y-4">
                ...
                <div>
                    <label htmlFor="treatmentDetails" className="block text-sm font-medium text-c-text-muted mb-1">Detalles del Tratamiento (Ej: Vacuna Triple)</label>
                    <textarea
                        id="treatmentDetails"
                        rows={3}
                        value={details}
                ...
                <div>
                    <label htmlFor="treatmentDate" className="block text-sm font-medium text-c-text-muted mb-1">Fecha de Aplicación</label>
                    <input
                        id="treatmentDate"
                        type="date"
                        value={date}
            ...
            <div className="space-y-4 pt-4">
                ...
                {treatmentsForLot.length > 0 ? (
                    ...
                    <div className="text-center py-10 bg-c-surface rounded-2xl">
                        <p className="text-c-text-faint">No hay tratamientos registrados para este lote.</p>
                    </div>
                )}
            </div>
        </div>
