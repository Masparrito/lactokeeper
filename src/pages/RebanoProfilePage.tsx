...
import { ArrowLeft, Edit, Save, X, Droplets, Scale, Syringe, Replace, CheckCircle, PlusCircle, Move, Tag, HeartPulse, Milk, FileText, Feather, AlertTriangle, ChevronRight, Archive, DollarSign, HeartCrack, Ban, RefreshCw, Trash2, Award, Baby } from 'lucide-react';
import { Modal } from '../components/ui/Modal';
import { ActionSheetModal, ActionSheetAction } from '../components/ui/ActionSheetModal';
import type { PageState } from '../types/navigation';
// --- TIPO 'Event' ELIMINADO ---
...
    'Nacimiento': { icon: Feather, color: 'bg-green-500/20 text-brand-green' },
    'Parto': { icon: Baby, color: 'bg-pink-500/20 text-pink-400' }, // <-- 'Baby' ahora está importado
...
                            <p className="text-xs text-zinc-500 mt-1">
                                {new Date(event.date + 'T00:00:00').toLocaleDateString('es-VE', { year: 'numeric', month: 'long', day: 'numeric' })}
...
            <InfoRow label="Fecha Nacimiento" value={new Date(animal.birthDate + 'T00:00:00').toLocaleDateString('es-VE')} isEditing={isEditing}>
...
            <InfoRow label="Categoría" value={zootecnicCategory} />
...
                    </div>
                    <p className="text-center text-brand-orange font-semibold text-sm -mt-1 mb-3 uppercase tracking-wider">{animal.isReference ? `Baja: ${animal.status}` : (animal.location || 'Sin Asignar')}</p>
