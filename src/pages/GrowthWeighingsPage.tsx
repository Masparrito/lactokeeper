...
import { ArrowLeft, Scale, Trash2, Pencil, Plus, X, Check, CalendarDays, TrendingUp } from 'lucide-react';
...
export default function GrowthWeighingsPage({ animalId, onBack }: GrowthWeighingsPageProps) {
    const { animals, bodyWeighings, addBodyWeighing, deleteBodyWeighing } = useData();
    ...
    return (
        <>
            <div className="w-full max-w-2xl mx-auto animate-fade-in px-4 pb-28">
                ...
                {/* Peso al nacer (contexto de solo lectura — es dato de la ficha) */}
                {birthWeight !== null && (
                    <div className="flex items-center gap-3 bg-c-surface-2/60 rounded-xl px-3 py-2.5 mb-4 border border-c-border">
                        <div className="w-9 h-9 rounded-lg bg-emerald-500/15 text-emerald-500 flex items-center justify-center flex-shrink-0"><Scale size={16} /></div>
                        <div className="min-w-0 flex-1">
                            <p className="text-sm font-semibold text-c-text-strong">Peso al nacer: {birthWeight.toFixed(2)} Kg</p>
                            <p className="text-xs text-c-text-faint">{birthDate ? fmtDate(birthDate) : ''} · dato de la ficha (se edita en el perfil del animal)</p>
                        </div>
                    </div>
                )}
