...
                        <h1 className="text-2xl font-bold tracking-tight text-c-text-strong">Gestionar Lotes Físicos</h1>
                        <p className="text-md text-c-text-muted">Crea ubicaciones y asigna animales</p>
                    </div>
                    <div className="w-8"></div>
section-end                 </header>

                <div className="px-4">
                    <div className="bg-c-surface backdrop-blur-xl rounded-2xl p-4 border border-c-border space-y-4">
                        <h3 className="text-lg font-semibold text-c-text-strong">Añadir Nuevo Lote</h3>
                        <div className="flex items-center gap-2">
                            <input type="text" value={newLotName} onChange={(e) => setNewLotName(e.target.value)} placeholder="Nombre del lote..." className="w-full bg-c-surface-2 p-3 rounded-xl text-lg text-c-text-strong placeholder-c-text-faint" />
                            <button onClick={handleAddLot} className="p-3 bg-c-accent hover:bg-c-accent/90 text-white rounded-xl"><Plus size={24} /></button>
                        </div>
...
                    <h3 className="text-lg font-semibold text-c-text-strong px-2">Lotes Existentes</h3>
                    {lotsWithCount.length > 0 ? (
                        lotsWithCount.map(lot => (
                            <div key={lot.id} className="w-full text-left bg-c-surface rounded-2xl p-4 flex justify-between items-center">
                                <div>
                                <p className="font-semibold text-c-text-strong">{lot.name}</p>
                                <p className="text-sm text-c-text-muted">{lot.animalCount} {lot.animalCount === 1 ? 'animal' : 'animales'}</p>
                            </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={() => handleOpenSelector(lot.name)} className="flex items-center gap-2 bg-c-surface-2 text-c-text-strong font-semibold py-2 px-3 rounded-lg hover:bg-c-surface-3 transition-colors"><Users size={16} /> Asignar</button>
                                    <button onClick={() => handleDeleteLot(lot.id, lot.animalCount)} className="p-2 text-c-text-faint hover:text-brand-red transition-colors"><Trash2 size={18} /></button>
section-end                                 </div>
                            </div>
                        ))
                    ) : (
                        <div className="text-center py-10 bg-c-surface rounded-2xl"><p className="text-c-text-faint">Aún no has creado ningún lote.</p></div>
                    )}
                </div>
            </div>
