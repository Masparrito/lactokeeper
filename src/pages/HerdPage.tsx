...
const getAnimalStatusObjects = (animal: Animal, allParturitions: Parturition[], allServiceRecords: ServiceRecord[], allBreedingGroups: BreedingGroup[]): (typeof STATUS_DEFINITIONS[AnimalStatusKey])[] => {
    ...
    if (animal.reproductiveStatus === 'Preñada') activeStatuses.push(STATUS_DEFINITIONS.PREGNANT);
        ...
        if (hasServiceRecord) activeStatuses.push(STATUS_DEFINITIONS.IN_SERVICE_CONFIRMED); else activeStatuses.push(STATUS_DEFINITIONS.IN_SERVICE);
    }
...
export default function HerdPage({ navigateTo, locationFilter }: HerdPageProps) {
    ...
    const animalsWithAllData = useMemo(() => {
        return animals.map(animal => ({ ...animal, statusObjects: getAnimalStatusObjects(animal, parturitions, serviceRecords, breedingGroups), formattedAge: formatAge(animal.birthDate), zootecnicCategory: getAnimalZootecnicCategory(animal, parturitions), }));
