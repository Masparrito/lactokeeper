...
// --- LÍNEA CORREGIDA: Se elimina 'ArrowLeft' de las importaciones ---
import { PlusCircle, Save, CheckCircle, AlertTriangle, X, Calendar, Zap, ScanLine } from 'lucide-react';
...
// --- MEJORA: Se elimina la prop 'onBack' ya que el Shell la maneja ---
        ...
        <>
            <div className="flex flex-col h-full">
                <div className="flex-shrink-0 space-y-4 pt-4">
                    {/* --- MEJORA: Se simplifica el header interno --- */}
                ...
                <div className="flex-shrink-0 p-4 border-t border-brand-border bg-gray-900/80 backdrop-blur-sm">
                    ...
                    <button onClick={handleFinalSave} className="w-full flex items-center justify-center gap-2 bg-brand-orange/20 border border-brand-orange text-brand-orange hover:bg-brand-orange/30 font-bold py-4 px-4 rounded-xl transition-colors text-lg">
...
export default function LactoKeeperAddDataPage({ onNavigate, onSaveSuccess }: LactoKeeperAddDataPageProps) {
    ...
    if (entryMode === 'rapid') {
        // --- MEJORA: Ya no se pasa 'onBack' al formulario ---
