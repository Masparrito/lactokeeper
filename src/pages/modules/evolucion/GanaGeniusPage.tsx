...
const CustomTooltip: React.FC<any> = ({ active, payload }) => { // 'label' no se usa
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-c-surface/95 p-3 rounded-lg border border-c-border shadow-lg backdrop-blur-md">
        <p className="text-sm font-bold text-c-text-strong">{month}</p>
        <hr className="border-c-border my-1.5" />
        <div className="space-y-1">
          {payload.map((entry: any) => (
            <div key={entry.name} className="flex justify-between items-center gap-4">
              <span className="text-xs" style={{ color: entry.color }}>{entry.name}:</span>
              <span className="text-xs font-mono text-c-text-strong">
...
const ComparativeLinearityChart: React.FC<{
  ...
  return (
    <div className="h-64 w-full bg-c-surface-2 p-4 rounded-lg border border-c-border">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
          <XAxis dataKey="name" stroke="#64748b" fontSize={12} />
          <YAxis stroke="#64748b" fontSize={12} tickFormatter={(val: number) => formatNumber(val, 0)} />
          <Tooltip content={<CustomTooltip />} />
          <Legend wrapperStyle={{ fontSize: '12px' }} />
          <Line type="monotone" name="CV Actual (L)" dataKey="CV Actual (L)" stroke="#94a3b8" strokeWidth={2} dot={false} />
          <Line type="monotone" name="CV Optimizado (L)" dataKey="CV Optimizado (L)" stroke="#2F843C" strokeWidth={2} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
  ...
  return (
    <div className="h-64 w-full bg-c-surface-2 p-4 rounded-lg border border-c-border">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
          <XAxis dataKey="name" stroke="#64748b" fontSize={12} tickFormatter={formatXAxis} />
          <YAxis stroke="#64748b" fontSize={12} tickFormatter={(val: number) => `${formatNumber(val / 1000, 0)}k`} />
          <Tooltip content={<CustomTooltip />} />
          <Legend wrapperStyle={{ fontSize: '12px' }} />
          <Line type="monotone" name="Ingreso Actual" dataKey="Ingreso Actual" stroke="#94a3b8" strokeWidth={2} dot={false} />
          <Line type="monotone" name={`Ingreso (+${kpiName})`} dataKey={`Ingreso (+${kpiName})`} stroke="#2F843C" strokeWidth={2} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
...
export const GanaGeniusPage: React.FC<GanaGeniusPageProps> = ({ 
  ...
  const renderDiagnosisResult = (report: UseSensitivityAnalysisResult['report']) => {
    ...
    return (
      <div className="p-4 max-w-2xl mx-auto space-y-6">
        <h2 className="text-2xl font-bold text-c-text-strong">Diagnóstico Zootécnico Completo</h2>
        ...
        <button
          ...
          className="w-full rounded-md bg-indigo-600 px-4 py-3 text-base font-medium text-white shadow-sm hover:bg-indigo-500"
        >
  ...
  const renderLinearityResult = () => {
    ...
    return (
      <div className="p-4 max-w-2xl mx-auto space-y-6">
        <h2 className="text-2xl font-bold text-c-text-strong">Optimización de Linealidad Completa</h2>
        ...
        {isBetter ? (
          <>
            ...
            <div className="flex justify-center gap-4 my-4">
              <div className="p-3 bg-c-surface-2 rounded-lg text-center">
                <p className="text-xs text-brand-red">CV Actual</p>
                <p className="text-2xl font-bold text-c-text-strong">{formatNumber(baseCV, 1)}%</p>
              </div>
              <div className="p-3 bg-c-accent/10 border border-c-accent/40 rounded-lg text-center">
                <p className="text-xs text-c-accent">CV Óptimo</p>
                <p className="text-2xl font-bold text-c-text-strong">{formatNumber(bestCV, 1)}%</p>
              </div>
            </div>

            <h3 className="font-semibold text-c-text-strong">Plan de Acción (Cupos de Monta):</h3>
            <p className="text-base font-semibold text-c-accent bg-c-surface-2 p-3 rounded-md mt-2">
              {suggestion}
            </p>
            ...
            <button
              ...
              className="w-full text-center text-sm text-c-text-muted hover:text-c-text-strong mt-4"
            >
  ...
  const renderModalContent = () => {
    ...
    if (error) {
      return (
        <div className="p-4 max-w-2xl mx-auto text-center">
          ...
          <button type="button" onClick={() => setModalStep('idle')} className="mt-6 w-full rounded-md bg-c-surface-2 px-4 py-2 text-sm font-medium text-c-text-strong">
            Volver
          </button>
        </div>
      );
    ...
    return (
      <div className="p-4 max-w-2xl mx-auto text-center">
        ...
        <h2 className="text-2xl font-bold text-c-text-strong mt-4">Asistente Zootécnico GanaGenius</h2>
        <p className="mt-2 text-lg text-c-text-muted">
        ...
        <ul className="text-left text-sm text-c-text-muted space-y-2 my-4 list-decimal list-inside">
        ...
        <button
          ...
          className="mt-4 w-full rounded-md bg-indigo-600 px-4 py-3 text-base font-medium text-white shadow-sm hover:bg-indigo-500"
        >
