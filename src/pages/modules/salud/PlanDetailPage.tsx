...
const SwipeableActivityCard = ({ activity, onEdit, onDelete }: { activity: PlanActivity, onEdit: () => void, onDelete: () => void }) => {
    ...
    return (
        <div className="relative w-full overflow-hidden rounded-2xl bg-c-surface-2/60 border border-c-border-strong">
            <div className="absolute inset-y-0 right-0 flex items-center z-0 h-full">
                <button onClick={onEdit} onPointerDown={e => e.stopPropagation()} className="h-full w-[80px] flex flex-col items-center justify-center bg-teal-600 text-white"><Edit size={20} /><span className="text-xs mt-1 font-semibold">Editar</span></button>
            ...
            <motion.div drag="x" dragConstraints={{ left: -buttonsWidth, right: 0 }} dragElastic={0.1} onDragStart={() => { dragStarted.current = true; }} onDragEnd={onDragEnd} animate={swipeControls} transition={{ type: "spring", stiffness: 400, damping: 40 }} className="relative w-full z-10 cursor-grab bg-c-surface p-4">
                <div className="flex justify-between items-start">
                    <div>
                        <p className="font-bold text-lg text-c-text">{activity.name}</p>
                        <p className={`text-sm font-semibold ${activity.category === 'Tratamiento' ? 'text-c-accent-sky' : 'text-purple-600'}`}>{activity.category}</p>
                        {product && <p className="text-xs text-c-text-muted mt-1">Ppal: {product.name}</p>}
                        {complementaryProduct && <p className="text-xs text-c-text-muted">Comp: {complementaryProduct.name}</p>}
                    </div>
                    {activity.category === 'Tratamiento' ? <Syringe className="text-c-accent-sky" /> : <ClipboardCheck className="text-purple-600" />}
                </div>
                <div className="mt-3 pt-3 border-t border-c-border flex items-center text-c-text-strong">
    ...
    if (!plan) {
        return (
            <div className="text-center p-10">
                <h1 className="text-2xl text-c-text-muted">Plan no encontrado.</h1>
                <button onClick={onBack} className="mt-4 text-c-accent-gold">Volver al Planificador</button>
            </div>
        );
    }
            ...
            <div className="w-full max-w-2xl mx-auto space-y-6 pb-12 animate-fade-in px-4">
                <header className="flex items-center pt-8 pb-4">
                    ...
                    <div className="text-center flex-grow">
                        <h1 className="text-3xl font-bold tracking-tight text-c-text">{plan.name}</h1>
                        <p className="text-lg text-c-text-muted">{plan.targetGroup}</p>
                    </div>
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        <h2 className="text-xl font-semibold text-c-text-strong">Actividades del Plan</h2>
                    ...
                    {activities.length > 0 ? (
                        ...
                        <div className="text-center py-10 bg-c-surface rounded-2xl"><p className="text-c-text-faint font-semibold">Este plan aún no tiene actividades.</p><p className="text-c-text-faint text-sm">Añade tratamientos o controles para empezar.</p></div>
                    )}
                </div>
            </div>
