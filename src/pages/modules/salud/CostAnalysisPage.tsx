...
const KpiCard = ({ title, value, unit, icon: Icon }: { title: string, value: string, unit?: string, icon: React.ElementType }) => (
    <div className="bg-c-surface backdrop-blur-xl rounded-2xl p-4 border border-c-border">
        ...
        <p className="text-3xl font-bold text-c-text mt-1">{value} <span className="text-xl text-c-text-muted">{unit}</span></p>
    </div>
);
    ...
    return (
        <div className="w-full max-w-2xl mx-auto space-y-4 animate-fade-in px-4 pb-12"> {/* Added pb-12 */}
            <header className="text-center pt-4"> {/* Added pt-4 */}
                <h1 className="text-3xl font-bold tracking-tight text-c-text">Análisis de Costos</h1>
                <p className="text-lg text-c-text-muted">Módulo StockCare</p>
            </header>
            ...
            <div className="bg-c-surface backdrop-blur-xl rounded-2xl p-4 border border-c-border">
                <h3 className="flex items-center gap-2 text-lg font-semibold text-c-text mb-4"><BarChart2 size={20}/> Costo Mensual (Últimos 6m)</h3>
                <div className="w-full h-56">
                    <ResponsiveContainer>
                        <BarChart data={monthlyCosts} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                            <XAxis dataKey="name" tickFormatter={formatMonth} tick={{ fill: '#64748b', fontSize: 12 }} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-c-surface backdrop-blur-xl rounded-2xl p-4 border border-c-border">
                    <h3 className="flex items-center gap-2 text-lg font-semibold text-c-text mb-4"><PieChartIcon size={20}/> Costos por Categoría</h3>
                 ...
                 <div className="bg-c-surface backdrop-blur-xl rounded-2xl p-4 border border-c-border">
                    <h3 className="text-lg font-semibold text-c-text mb-4">Top 5 Animales (Costo Total)</h3>
                    <div className="space-y-2">
                        {topCostAnimals.length > 0 ? topCostAnimals.map((animal, index) => (
                            <div key={animal.animalId} className="flex justify-between items-center text-sm bg-c-surface-2/60 p-2 rounded-md">
                                {/* --- CAMBIO: Aplicado font-mono y text-base al ID --- */}
                                <span className="font-mono font-semibold text-base text-c-text">
                                    {index + 1}. {animal.animalId}
                                </span>
                                <span className="font-bold text-teal-600">${animal.total.toFixed(2)}</span>
                            </div>
                        )) : <p className="text-sm text-c-text-faint text-center pt-8">No hay datos suficientes.</p>}
                    </div>
                </div>
            </div>
        </div>
