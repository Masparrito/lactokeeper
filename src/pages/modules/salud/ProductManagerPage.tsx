        ...
        <>
            <div className="w-full max-w-2xl mx-auto space-y-6 animate-fade-in px-4">
                <header className="text-center">
                    <h1 className="text-3xl font-bold tracking-tight text-c-text">Inventario de Productos</h1>
                    <p className="text-lg text-c-text-muted">Gestión de Insumos Sanitarios</p>
                </header>
                ...
                <div className="space-y-2 pt-4">
                     <h3 className="flex items-center gap-2 text-lg font-semibold text-c-text-strong"><Filter size={18}/> Filtrar por Categoría</h3>
                     <div className="flex flex-wrap gap-2">
                        {productCategories.map(category => (
                            <button
                                key={category}
                                onClick={() => setActiveFilter(category as any)}
                                className={`px-3 py-1.5 text-sm font-semibold rounded-full transition-colors ${activeFilter === category ? 'bg-teal-600 text-white' : 'bg-c-surface-2 text-c-text-strong'}`}
                            >
                                {category}
                            </button>
                        ))}
                ...
                <div className="space-y-3 pt-4">
                    {filteredProducts.length > 0 ? (
                        filteredProducts.map(product => (
                            <div key={product.id} className="bg-c-surface rounded-2xl p-4 border border-c-border">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="font-bold text-lg text-c-text">{product.name}</p>
                                        <p className="text-sm text-c-text-muted">{product.presentationValue ? `${product.presentationValue} ${product.presentationUnit}` : 'Sin presentación'}</p>
                                        <div className="mt-2 flex items-center flex-wrap gap-x-4 gap-y-1 text-xs">
                                            ...
                                            <span className="flex items-center gap-1 font-semibold text-amber-600"><DollarSign size={14}/> {getCostPerUnit(product)}</span>
                                        </div>
                                    </div>
                                    <div className="flex gap-2 flex-shrink-0">
                                        <button onClick={() => handleOpenModal(product)} className="p-2 text-c-text-muted hover:text-teal-600 transition-colors"><Edit size={18} /></button>
                                        <button onClick={() => setDeleteConfirmation(product)} className="p-2 text-c-text-muted hover:text-brand-red transition-colors"><Trash2 size={18} /></button>
                                    </div>
                                </div>
                            </div>
                        ))
                        ...
                        <div className="text-center py-10 bg-c-surface rounded-2xl">
                            <p className="text-c-text-faint font-semibold">No se encontraron productos.</p>
                             <p className="text-c-text-faint text-sm">{activeFilter === 'Todos' ? 'Añade tu primer producto para empezar.' : `No hay productos en la categoría "${activeFilter}".`}</p>
                        </div>
                    )}
                </div>
            </div>
