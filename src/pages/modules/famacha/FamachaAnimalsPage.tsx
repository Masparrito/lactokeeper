...
export function FamachaAnimalsPage() {
    ...
    return (
        <div className="w-full max-w-2xl mx-auto px-4 space-y-3">
            <p className="text-sm text-zinc-400 px-1">Toca un animal para ver todo su historial Famacha.</p>
            ...
            <div className="bg-brand-glass rounded-2xl border border-brand-border divide-y divide-zinc-800 overflow-hidden">
                {filas.map(f => (
                    <button key={f.animalId} onClick={() => f.ultima && setOpenId(f.animalId)} className="w-full flex items-center gap-3 px-3 py-3 text-left hover:bg-zinc-800/40 transition-colors">
                        <span className={`w-1 self-stretch rounded-full ${f.ultima ? scoreBar[f.ultima.score] : 'bg-zinc-700'}`} />
                        <div className="min-w-0 flex-1">
                            <span className="font-bold text-white">{f.arete}</span>
                        ...
                        {f.ultima && <ChevronRight size={18} className="text-zinc-600 flex-shrink-0" />}
                    </button>
                ))}
                {filas.length === 0 && <p className="text-center text-zinc-500 py-8">Sin animales.</p>}
            </div>

            {/* Modal historial */}
            {detalle && (
                <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end justify-center" onClick={() => setOpenId(null)}>
                    <div className="w-full max-w-lg bg-ios-modal-bg rounded-t-2xl p-5 max-h-[80vh] overflow-y-auto animate-slide-up" onClick={e => e.stopPropagation()} style={{ paddingBottom: 'env(safe-area-inset-bottom, 1.25rem)' }}>
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="font-bold text-white text-xl">{detalle.arete}</h3>
                            <button onClick={() => setOpenId(null)} className="text-zinc-500 hover:text-white"><X size={22} /></button>
                        </div>

                        {alertaDetalle && (
                            <div className="flex gap-2 bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 mb-4">
                                <AlertTriangle size={18} className="text-amber-400 flex-shrink-0 mt-0.5" />
                                <p className="text-xs text-amber-200 leading-relaxed">{MENSAJE_NO_MEJORA}</p>
                            </div>
                        )}

                        <div className="space-y-2">
                            {detalle.revs.map(r => (
                                <div key={r.id} className="flex items-center gap-3 bg-black/20 rounded-xl p-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <p className="text-white text-sm font-semibold">{r.fecha} · {accionLabel[r.accion] || r.accion}</p>
