                ...
                {dates.map(date => {
                    ...
                    return (
                        <button key={date.toISOString()} type="button" onClick={() => onDateChange(date)}
                            className={`flex flex-col items-center justify-center rounded-lg p-2 w-16 h-16 flex-shrink-0 transition-colors ${isSelected ? 'bg-c-accent text-white' : 'bg-c-surface-2 text-c-text-strong hover:bg-c-surface-3'}`}>
        ...
        <div className={`p-3 rounded-lg flex justify-between items-center animate-fade-in group ${isUnrecognized ? 'bg-amber-900/40 border border-amber-500/50' : 'bg-c-surface-2'}`}>
            <div className="flex items-center space-x-2 min-w-0 pr-2">
                ...
                <div className="min-w-0">
                    ...
                    {formattedName && (
                        <p className="text-sm font-normal text-c-text-muted truncate">{formattedName}</p>
                    )}
                </div>
            </div>
        ...
        <>
            <div className="flex flex-col h-full animate-fade-in">
                <div className="flex-shrink-0 space-y-4 pt-4">
                    <header className="text-center px-4 flex items-center">
                        ...
                        <div className="flex-grow">
                            <h1 className="text-2xl font-bold tracking-tight text-c-text-strong">Carga Rápida de Peso</h1>
                            <p className="text-md text-c-text-muted">Sesión de Pesaje Corporal</p>
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-2 px-4">
                            <input ref={idInputRef} type="text" value={currentId} onChange={(e) => setCurrentId(e.target.value)} onKeyDown={handleIdKeyDown} placeholder="ID Animal" className="w-full bg-c-surface p-4 rounded-xl text-lg text-c-text placeholder-c-text-faint border-2 border-transparent focus:border-c-accent focus:ring-0"/>
                            <input ref={kgInputRef} type="number" step="0.1" value={currentKg} onChange={(e) => setCurrentKg(e.target.value)} onKeyDown={handleKgKeyDown} placeholder="Kg" className="w-32 flex-shrink-0 bg-c-surface p-4 rounded-xl text-lg text-c-text placeholder-c-text-faint border-2 border-transparent focus:border-c-accent focus:ring-0"/>
                            <button onClick={handleAddToList} disabled={!currentId || !currentKg || isLoading} aria-label="Añadir a la lista" className="aspect-square h-full bg-c-accent text-white rounded-xl flex items-center justify-center transition-all hover:bg-c-accent/90 disabled:opacity-40"><PlusCircle size={24} /></button>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-shrink-0 p-4 border-t border-c-border bg-c-surface/80 backdrop-blur-sm">
                    ...
                    {isLoading && ( <div className={`mb-3 flex items-center justify-center space-x-2 p-3 rounded-lg text-sm bg-c-surface-2/50 text-c-text-strong`}><Loader2 size={18} className="animate-spin" /> <span>Guardando...</span> </div> )}

                    <button
                        onClick={handleFinalSave}
                        disabled={isLoading || sessionEntries.length === 0 || sessionEntries.some(e => !e.isRecognized)}
                        className="w-full flex items-center justify-center gap-2 bg-c-accent/20 border border-c-accent text-c-accent hover:bg-c-accent/30 font-bold py-4 px-4 rounded-xl transition-colors text-lg disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        <Save size={20} />
