...
export default function AddAnimalPage({ onBack }: AddAnimalPageProps) {
    return (
        ...
        <div 
            className="fixed inset-0 z-50 bg-c-bg/60 backdrop-blur-md flex flex-col justify-end animate-fade-in"
            onClick={onBack} // Permite cerrar el modal al tocar fuera del contenido
        >
            {/* Contenedor del contenido que se desliza desde abajo */}
            <div 
                className="bg-c-surface w-full max-w-4xl mx-auto h-[95vh] rounded-t-2xl flex flex-col animate-slide-up"
                onClick={(e) => e.stopPropagation()} // Evita que el modal se cierre al tocar dentro
            >
                {/* Encabezado del Modal */}
                <header className="flex-shrink-0 flex items-center justify-between p-4 border-b border-c-border">
                    <div className="w-10"></div> {/* Espaciador para centrar el título */}
                    <h1 className="text-xl font-bold tracking-tight text-c-text">Ingresar Nuevo Animal</h1>
                    <button
                        onClick={onBack}
                        className="p-2 text-c-text-muted hover:text-c-text rounded-full hover:bg-c-surface-2 transition-colors"
                        aria-label="Cerrar"
                    >
                        <X size={24} />
                    </button>
