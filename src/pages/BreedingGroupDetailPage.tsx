    ...
    return (
        <div className="w-full text-left bg-zinc-800/50 rounded-2xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <div className="w-full sm:w-auto flex flex-shrink-0 gap-2">
                <button onClick={() => onDeclareService(animal.id)} className="flex-1 sm:flex-initial bg-indigo-600/80 hover:bg-indigo-500/100 text-white font-semibold py-2 px-3 rounded-lg transition-colors text-sm">Servicio</button>
                <button onClick={() => onDeclareParturition(animal.id)} className="flex-1 sm:flex-initial flex items-center justify-center gap-2 bg-green-600/80 hover:bg-green-500/100 text-white font-semibold py-2 px-3 rounded-lg transition-colors text-sm"><Feather size={14} /> Parto</button>
            </div>
        </div>
    );
};
...
export default function BreedingGroupDetailPage({ groupId, navigateTo, onBack }: BreedingGroupDetailPageProps) {
    ...
    const handleAssignFemales = async (selectedIds: string[]) => {
        ...
        const updatePromises = selectedIds.map(animalId => {
            return updateAnimal(animalId, { breedingGroupId: group.id, reproductiveStatus: 'En Servicio' });
        });
    ...
    const handleDeclareService = async (date: Date | undefined) => {
        ...
        await addServiceRecord({ breedingGroupId: group.id, femaleId: serviceModal.femaleId, serviceDate: date.toISOString().split('T')[0] });
        ...
        <>
            <div className="w-full max-w-2xl mx-auto space-y-4 pb-12">
                <header className="flex items-center pt-8 pb-4">
                    ...
                    <div className="text-center flex-grow"><h1 className="text-2xl font-bold tracking-tight text-white">{group.name}</h1><p className="text-md text-zinc-400">Reproductor: {group.sireId}</p></div>
                    <div className="w-8"></div>
                </header>
                <button onClick={() => setSelectorOpen(true)} className="w-full flex items-center justify-center gap-2 bg-brand-orange hover:bg-orange-600 text-white font-bold py-4 px-4 rounded-xl transition-colors text-lg"><Plus size={20} /> Asignar Hembras</button>
                <div className="space-y-4 pt-4">
                    <div className="flex justify-between items-center px-2">
                        <h3 className="text-lg font-semibold text-zinc-300">Hembras Asignadas ({filteredFemales.length})</h3>
                        <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-500" /><input type="search" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Buscar..." className="w-full bg-zinc-800/80 rounded-lg pl-10 pr-4 py-2 text-white border-transparent focus:border-brand-amber focus:ring-0" /></div>
                    </div>
            ...
            <AdvancedAnimalSelector 
                ...
                title={`Asignar Hembras a: ${group.name}`} 
                sireIdForInbreedingCheck={group.sireId} 
            />
            ...
            <Modal isOpen={parturitionModal.isOpen} onClose={() => setParturitionModal({ isOpen: false, femaleId: null })} title={`Registrar Parto para ${parturitionModal.femaleId}`}>
                <AddParturitionForm motherId={parturitionModal.femaleId || ''} onSaveSuccess={() => setParturitionModal({ isOpen: false, femaleId: null })} onCancel={() => setParturitionModal({ isOpen: false, femaleId: null })} />
            </Modal>
        </>
    );
}