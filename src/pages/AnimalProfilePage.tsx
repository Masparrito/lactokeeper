...
const ModalChartTooltip = ({ active, payload, label, context }: { active?: boolean, payload?: any[], label?: string, context: 'peak' | 'del' | 'interval' | 'average' }) => {
    if (active && payload && payload.length) {
        ...
        switch (context) {
            case 'average':
                title = `Lactancia del ${label}`;
                valueText = `Promedio: ${payload[0].value.toFixed(2)} Kg`;
                break;
            case 'peak':
                title = `Lactancia del ${label}`;
                valueText = `Pico: ${payload[0].value.toFixed(2)} Kg`;
                break;
            case 'del':
                title = `Lactancia del ${label}`;
                valueText = `Duración: ${payload[0].value} días`;
                break;
            case 'interval':
                title = `Período ${label}`;
                valueText = `Intervalo: ${payload[0].value} días`;
                break;
        }
  ...
  return (
    <>
      <div className="w-full max-w-4xl mx-auto space-y-4 animate-fade-in">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <button onClick={() => setActiveModal('del')} className="bg-brand-glass backdrop-blur-xl rounded-2xl p-3 border border-brand-border text-left hover:border-brand-amber transition-colors">
              ...
              <p className="text-2xl font-bold text-white">{currentLactation?.totalDays || 'N/A'}</p>
            </button>
      ...
      <Modal isOpen={activeModal === 'average'} onClose={() => setActiveModal(null)} title="Historial de Promedios">
        <div className="space-y-6">
            <div className="w-full h-48 -ml-4"><ResponsiveContainer>
                <BarChart data={chartData.average} margin={{ top: 5, right: 20, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="name" tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 12 }} stroke="transparent" tickLine={false} />
                    <YAxis orientation="right" tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 12 }} stroke="transparent" tickLine={false} width={40} />
                    <Tooltip content={<ModalChartTooltip context="average" />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} />
                    <Bar dataKey="value" radius={[4, 4, 0, 0]} fill="url(#modalBarGradient)" />
                </BarChart>
            </ResponsiveContainer></div>
            <div>
                <h4 className="text-sm font-semibold text-zinc-400 mb-2">Detalles por Lactancia</h4>
                <ul className="space-y-1">{allLactations.slice().reverse().map(lact => (
                    <li key={lact.parturitionDate} className="flex justify-between items-center p-2 rounded-md hover:bg-white/5"><span className="text-base font-medium text-zinc-200">Parto del {lact.parturitionDate}</span><span className="text-base font-semibold text-white">{lact.averageProduction.toFixed(2)} Kg</span></li>
                ))}</ul>
            </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={activeModal === 'peak'} onClose={() => setActiveModal(null)} title="Historial de Picos de Producción">
        <div className="space-y-6">
            <div className="w-full h-48 -ml-4"><ResponsiveContainer>
                <BarChart data={chartData.peak} margin={{ top: 5, right: 20, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="name" tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 12 }} stroke="transparent" tickLine={false} />
                    <YAxis orientation="right" tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 12 }} stroke="transparent" tickLine={false} width={40} />
                    <Tooltip content={<ModalChartTooltip context="peak" />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} />
                    <Bar dataKey="value" radius={[4, 4, 0, 0]} fill="url(#modalBarGradientPeak)" />
                </BarChart>
            </ResponsiveContainer></div>
            <div>
                <h4 className="text-sm font-semibold text-zinc-400 mb-2">Detalles por Lactancia</h4>
                <ul className="space-y-1">{allLactations.slice().reverse().map(lact => (
                    <li key={lact.parturitionDate} className="flex justify-between items-center p-2 rounded-md hover:bg-white/5"><span className="text-base font-medium text-zinc-200">Parto del {lact.parturitionDate}</span><span className="text-base font-semibold text-white">{lact.peakProduction.kg.toFixed(2)} Kg <span className="font-normal text-zinc-400">(Día {lact.peakProduction.del})</span></span></li>
                ))}</ul>
            </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={activeModal === 'del'} onClose={() => setActiveModal(null)} title="Historial de Duración de Lactancias">
        <div className="space-y-6">
            <div className="w-full h-48 -ml-4"><ResponsiveContainer>
                <BarChart data={chartData.del} margin={{ top: 5, right: 20, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="name" tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 12 }} stroke="transparent" tickLine={false} />
                    <YAxis orientation="right" tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 12 }} stroke="transparent" tickLine={false} width={40} />
                    <Tooltip content={<ModalChartTooltip context="del" />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} />
                    <Bar dataKey="value" radius={[4, 4, 0, 0]} fill="url(#modalBarGradientDel)" />
                </BarChart>
            </ResponsiveContainer></div>
            <div>
                <h4 className="text-sm font-semibold text-zinc-400 mb-2">Detalles por Lactancia</h4>
                <ul className="space-y-1">{allLactations.slice().reverse().map(lact => (
                    <li key={lact.parturitionDate} className="flex justify-between items-center p-2 rounded-md hover:bg-white/5"><span className="text-base font-medium text-zinc-200">Parto del {lact.parturitionDate}</span><span className="text-base font-semibold text-white">{lact.totalDays} días</span></li>
                ))}</ul>
            </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={activeModal === 'interval'} onClose={() => setActiveModal(null)} title="Historial de Intervalos entre Partos">
        <div className="space-y-6">
            <div className="w-full h-48 -ml-4"><ResponsiveContainer>
                <BarChart data={chartData.interval} margin={{ top: 5, right: 20, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="name" tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 12 }} stroke="transparent" tickLine={false} />
                    <YAxis orientation="right" tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 12 }} stroke="transparent" tickLine={false} width={40} />
                    <Tooltip content={<ModalChartTooltip context="interval" />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} />
                    <Bar dataKey="value" radius={[4, 4, 0, 0]} fill="url(#modalBarGradientInt)" />
                </BarChart>
            </ResponsiveContainer></div>
            <div>
                <h4 className="text-sm font-semibold text-zinc-400 mb-2">Detalles de Intervalos</h4>
                <ul className="space-y-1">{parturitionIntervals.slice().reverse().map(interval => (
                    <li key={interval.period} className="flex justify-between items-center p-2 rounded-md hover:bg-white/5"><span className="text-base font-medium text-zinc-200">Período {interval.period}</span><span className="text-base font-semibold text-white">{interval.days} días</span></li>
                ))}</ul>
            </div>
        </div>
      </Modal>
