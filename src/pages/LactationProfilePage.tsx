            ...
            <div className="w-full max-w-4xl mx-auto space-y-4 animate-fade-in px-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <div className="bg-brand-glass backdrop-blur-xl rounded-2xl p-3 border border-brand-border text-left">
                        ...
                        <p className="text-xl font-bold text-white">{lastWeighingDate ? new Date(lastWeighingDate + 'T00:00:00').toLocaleDateString() : 'N/A'}</p>
                    </div>
                </div>
                    ...
                    <div className="bg-brand-glass backdrop-blur-xl rounded-2xl p-4 border border-brand-border">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            {currentLactationData.status === 'activa' && (
                                <button
                                    onClick={() => startDryingProcess(currentLactationData.id!)}
                                    className="w-full flex items-center justify-center space-x-2 bg-blue-600/80 hover:bg-blue-500/80 text-white font-semibold py-3 px-4 rounded-xl transition-colors"
                                >
                                    <Wind size={20} />
                                    <span>Iniciar Proceso de Secado</span>
                                </button>
                            )}
                            {currentLactationData.status === 'en-secado' && (
                                <button
                                    onClick={() => setLactationAsDry(currentLactationData.id!)}
                                    className="w-full flex items-center justify-center space-x-2 bg-gray-600/80 hover:bg-gray-500/80 text-white font-semibold py-3 px-4 rounded-xl transition-colors"
                                >
                                    <Archive size={20} />
                                    <span>Declarar Lactancia como Seca</span>
                                </button>
                            )}
                            {currentLactationData.status === 'seca' && (
                                <div className="w-full text-center p-3 bg-black/20 rounded-xl">
                                    <p className="font-semibold text-green-400">Esta lactancia ha finalizado.</p>
                                </div>
                            )}
                        </div>
                        {currentLactationData.status === 'en-secado' && (
