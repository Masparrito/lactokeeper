...
    sireId: string;
    sireName: string; // <-- Este campo es la solución
}
...
                // --- CORRECCIÓN: Se cambia el fallback de 'lot.sireId' a 'Desconocido' ---
...
            };
        }).sort((a, b) => {
            if (a.projectedStartDate && b.projectedStartDate) {
                return a.projectedStartDate.getTime() - b.projectedStartDate.getTime();
            } else if (a.projectedStartDate) {
                return -1; // a va primero si b no tiene fecha
            } else if (b.projectedStartDate) {
                return 1; // b va primero si a no tiene fecha
            }
...
                if(seasonForecasts[i].projectedStartDate && seasonForecasts[i-1].projectedEndDate) {
