...
    return (
      // --- LÍNEA CORREGIDA ---
      <Modal isOpen={isOpen} onClose={onClose} title={`Acción Requerida: ${animal.id}`}>
...
      return <ParturitionModal 
        isOpen={true} // El modal de parto gestiona su propia visibilidad (pero se la pasamos)
        onClose={() => {
...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Pesaje Lechero: ${animal.id}`}>
...
                    <span className="text-base font-semibold text-white">{new Date(date + 'T00:00:00').toLocaleDateString('es-VE', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                  </div>
...
            className="w-full text-left bg-brand-orange/20 border border-brand-orange/80 p-4 rounded-xl hover:bg-brand-orange/30 transition-colors flex justify-between items-center"
          >
...
                        disabled={activeParturition.status === 'en-secado'} 
...
                        className="w-full flex items-center justify-center gap-2 bg-gray-600/20 text-gray-300 font-semibold py-3 px-3 rounded-lg hover:bg-gray-600/40 transition-colors"
                    >
