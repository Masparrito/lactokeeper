...
const ActivityEditor = ({ targetGroup, activityType, onSaveActivity, existingActivity, clearEditing }: { targetGroup: 'Maternidad' | 'Adultos', activityType: ActivityType, onSaveActivity: (activity: Omit<PlanActivity, 'id' | 'healthPlanId'>, isEditing: boolean) => void, existingActivity: Omit<PlanActivity, 'id' | 'healthPlanId'> | null, clearEditing: () => void }) => {
    ...
    return (
        <div className="space-y-6">
            <h3 className="text-xl font-semibold text-c-text-strong">Añadir {activityType}</h3>

            <div className="space-y-4">
                <div className="flex items-center gap-2">
                    <input list="activity-names-list" value={name} onChange={e => setName(e.target.value)} placeholder={`Nombre del ${activityType}...`} className="w-full bg-c-surface-2 text-c-text p-3 rounded-lg text-base" />
                    <datalist id="activity-names-list">{nameSuggestions.map(s => <option key={s} value={s} />)}</datalist>
                    {activityType === 'Control' && <button type="button" onClick={() => alert('Funcionalidad para crear nuevos tipos de control en desarrollo.')} className="flex-shrink-0 p-3 bg-teal-600/80 text-white rounded-lg"><PlusCircle size={20} /></button>}
                </div>

                {activityType === 'Tratamiento' && (
                    <div className="space-y-2">
                        <div className="flex items-center gap-2">
                            <select value={productId} onChange={e => setProductId(e.target.value)} className="w-full bg-c-surface-2 text-c-text p-3 rounded-lg text-base appearance-none"><option value="" disabled>Producto Principal...</option>{Object.entries(groupedProducts).map(([category, productList]) => ( <optgroup key={category} label={category}>{productList.map(p => <option key={p.id} value={p.id!}>{p.name}</option>)}</optgroup>))}</select>
                            <button type="button" onClick={() => setIsProductModalOpen(true)} className="flex-shrink-0 p-3 bg-teal-600 text-white rounded-lg"><PlusCircle size={20} /></button>
                        </div>
                         <div className="flex items-center gap-2">
                            <select value={complementaryProductId} onChange={e => setComplementaryProductId(e.target.value)} className="w-full bg-c-surface-2 text-c-text p-3 rounded-lg text-base appearance-none"><option value="">Producto Complementario (Opcional)...</option>{Object.entries(groupedProducts).map(([category, productList]) => ( <optgroup key={category} label={category}>{productList.map(p => <option key={p.id} value={p.id!}>{p.name}</option>)}</optgroup>))}</select>
                            <button type="button" onClick={() => setIsProductModalOpen(true)} className="flex-shrink-0 p-3 bg-teal-600/50 text-white rounded-lg"><PlusCircle size={20} /></button>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="space-y-3">
                <label className="text-base font-medium text-c-text-muted">Tiempos de Aplicación (Selecciona uno o varios)</label>
                {targetGroup === 'Maternidad' ? (
                    <>
                        <div className="flex flex-wrap gap-2">
                            {PRESET_DAYS.map(day => <button type="button" key={day} onClick={() => handleTimeToggle(day)} className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${selectedTimes.has(day) ? 'bg-zinc-200 text-black' : 'bg-c-surface-2 text-c-text-strong'}`}>{day === '0' ? 'Nacimiento' : `${day} días`}</button>)}
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            <input type="number" value={manualTime} onChange={e => setManualTime(e.target.value)} placeholder="Día manual..." className="w-full bg-c-surface-2 text-c-text p-3 rounded-lg text-base" />
                            <button type="button" onClick={handleAddManualTime} className="px-5 py-3 bg-c-surface-2 text-c-text font-semibold rounded-lg text-sm hover:bg-c-surface-3">Agregar</button>
                        </div>
                    </>
                ) : (
                    <div className="p-2 bg-c-surface-2 rounded-lg">
                           ...
                           {!expandedMonth ? (
                                <motion.div key="months-view" initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} className="grid grid-cols-4 gap-2">
                                    {[...Array(12)].map((_, monthIndex) => {
                                        ...
                                        return (
                                            <button type="button" key={monthNumber} onClick={() => setExpandedMonth(monthNumber)} className="relative text-center py-3 px-2 text-sm font-semibold rounded-md bg-c-surface-2 hover:bg-c-surface-3 text-c-text-strong transition-colors">
                                ...
                                <motion.div key="weeks-view" initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} className="space-y-3">
                                    <div className="flex items-center gap-2">
                                        <button type="button" onClick={() => setExpandedMonth(null)} className="p-2 text-c-text-muted hover:bg-c-surface-3 rounded-md"><ArrowLeft size={16}/></button>
                                        <h4 className="font-bold text-c-text-strong">{new Date(0, expandedMonth - 1).toLocaleString('es-VE', {month: 'long'})}</h4>
                                    </div>
                                    <div className="grid grid-cols-4 gap-2">
                                        {[1, 2, 3, 4].map(week => {
                                            ...
                                            return (
                                                <button type="button" key={week} onClick={() => handleTimeToggle(timeValue)} className={`py-3 px-2 text-sm font-semibold rounded-md transition-colors ${isSelected ? 'bg-teal-500 text-white' : 'bg-c-surface-2 text-c-text-strong hover:bg-c-surface-3'}`}>
                                                    Semana {week}
                                                </button>
                                            );
                                        })}
            ...
            <button type="button" onClick={handleSaveClick} className="w-full py-3 bg-teal-600 text-white font-bold rounded-lg text-base mt-4">{existingActivity ? 'Actualizar Actividad' : 'Añadir a la Lista'}</button>
            ...
            return (
                <motion.div key="step1" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 50 }} transition={{ duration: 0.3 }} className="space-y-6">
                    <div className="text-center">
                        <h2 className="text-2xl sm:text-3xl font-bold text-c-text-strong">Crear Plan Sanitario</h2>
                        <p className="text-c-text-muted">Paso 1: Define tu plan</p>
                    </div>
                    <div className="space-y-4">
                        <input type="text" value={planName} onChange={e => setPlanName(e.target.value)} placeholder="Nombre del Plan" className="w-full bg-c-surface-2 text-c-text p-3 rounded-xl text-base" />
                        <input type="text" value={planDescription} onChange={e => setPlanDescription(e.target.value)} placeholder="Descripción (Opcional)" className="w-full bg-c-surface-2 text-c-text p-3 rounded-xl text-base" />

                        <div>
                            <label className="text-base font-medium text-c-text-muted mb-2 block">Grupo Objetivo</label>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <button type="button" onClick={() => setTargetGroup('Maternidad')} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${targetGroup === 'Maternidad' ? 'bg-teal-500/20 border-teal-500 scale-105' : 'bg-c-surface-2 border-c-border'}`}>
                                    <FaBaby className="w-8 h-8 text-teal-400" />
                                    <span className="font-semibold text-c-text">Maternidad</span>
                                </button>
                                <button type="button" onClick={() => setTargetGroup('Adultos')} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${targetGroup === 'Adultos' ? 'bg-pink-500/20 border-pink-500 scale-105' : 'bg-c-surface-2 border-c-border'}`}>
                                    <FaTractor className="w-8 h-8 text-pink-400" />
                                    <span className="font-semibold text-c-text">Rebaño Adulto</span>
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row-reverse sm:justify-start gap-3 pt-4">
                        <button onClick={() => { if(planName.trim()) { setStep(2); setError(null); } else { setError("El nombre del plan es obligatorio.")}}} className="w-full sm:w-auto px-5 py-3 bg-teal-600 text-white font-bold rounded-xl flex items-center justify-center gap-2">Siguiente</button>
                        <button onClick={onCancel} className="w-full sm:w-auto px-5 py-3 bg-c-surface-2 text-c-text hover:bg-c-surface-3 font-semibold rounded-xl">Cancelar</button>
                    </div>
                </motion.div>
            );
        }
        ...
        if (step === 2) {
            return (
                <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                    <div className="text-center">
                        <h2 className="text-2xl sm:text-3xl font-bold text-c-text-strong">{planName || 'Nuevo Plan'}</h2>
                        <p className="text-c-text-muted">Paso 2: Añade las actividades para el grupo: <span className="font-semibold text-c-text-strong">{targetGroup}</span></p>
                    </div>

                    <AnimatePresence>
                        {targetGroup === 'Adultos' && (
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-2 overflow-hidden">
                                <label className="text-base font-medium text-c-text-muted mb-2 block">Categorías (Selecciona una o varias)</label>
                                <div className="flex flex-wrap gap-2">
                                    {ADULT_SUBGROUPS.map(subgroup => (
                                        <button key={subgroup} type="button" onClick={() => handleSubgroupToggle(subgroup)} className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${selectedSubgroups.has(subgroup) ? 'bg-pink-500 text-white' : 'bg-c-surface-2 text-c-text-strong'}`}>
                                            {subgroup}
                                        </button>
                                    ))}
                                </div>
                    ...
                    <div className="space-y-3">
                        <h3 className="text-xl font-semibold text-c-text-strong">Actividades del Plan ({activities.length})</h3>
                        <div className="min-h-[100px] max-h-60 overflow-y-auto pr-2 border border-c-border rounded-xl p-2 space-y-2">
                            <AnimatePresence>
                                {activities.length > 0 ? (
                                    activities.map((act) => (
                                        <motion.div key={act.tempId} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`flex justify-between items-center bg-c-surface-2 p-3 rounded-lg border-l-4 ${act.category === 'Tratamiento' ? 'border-blue-500' : 'border-purple-500'}`}>
                                            <div>
                                                <p className="font-semibold text-c-text-strong">{act.name}</p>
                                                <p className="text-xs text-c-text-muted">{act.trigger.days?.map(d => d === 0 ? 'Nacer' : d).join(', ') || 'Tiempos no definidos'} días</p>
                                            </div>
                                            <div className="flex gap-2">
                                                <button onClick={() => setEditingActivity({ activity: act, tempId: act.tempId })} className="p-1 text-c-text-muted hover:text-teal-600"><Edit size={16}/></button>
                                                <button onClick={() => handleRemoveActivity(act.tempId)} className="p-1 text-c-text-muted hover:text-brand-red"><Trash2 size={16}/></button>
                                            </div>
                                        </motion.div>
                                    ))
                                ) : (
                                    <div className="text-center py-6 text-c-text-faint"><p>Añade actividades para empezar a construir tu plan.</p></div>
                                )}
                            </AnimatePresence>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="relative flex w-full max-w-sm mx-auto justify-between rounded-lg bg-c-surface-2 p-1 mb-6">
                            <button type="button" onClick={() => setActivityTypeToAdd('Tratamiento')} className="relative z-10 w-1/2 py-2 text-sm font-semibold text-c-text flex items-center justify-center gap-2"><FaSyringe/> Tratamiento</button>
                            <button type="button" onClick={() => setActivityTypeToAdd('Control')} className="relative z-10 w-1/2 py-2 text-sm font-semibold text-c-text flex items-center justify-center gap-2"><FaStethoscope/> Control</button>
                            <motion.div layoutId="add-type-bg" transition={{ type: "spring", stiffness: 400, damping: 30 }} className={`absolute top-1 h-[calc(100%-0.5rem)] w-1/2 rounded-md bg-c-surface-3 ${activityTypeToAdd === 'Control' ? 'left-1/2' : 'left-1'}`} />
                        </div>
                    ...
                    <div className="flex flex-col-reverse sm:flex-row sm:justify-between items-center pt-6 gap-4">
                        <button onClick={() => setStep(1)} className="w-full sm:w-auto px-5 py-3 text-c-text-strong font-semibold rounded-xl flex items-center justify-center gap-2 hover:bg-c-surface-2"><ArrowLeft size={16}/> Atrás</button>
                        <div className="flex gap-3 w-full sm:w-auto">
                            <button onClick={() => setIsPreviewOpen(true)} className="flex-1 px-5 py-3 bg-teal-600 text-white font-bold rounded-xl flex items-center justify-center gap-2"><Eye size={16}/> Previsualizar</button>
