  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="text-center mb-2">
            <p className="font-mono font-semibold text-xl text-c-text truncate">{animalId.toUpperCase()}</p>
            {formattedName && (
      ...
      <div>
        <label htmlFor="weighingDate" className="block text-sm font-medium text-c-text-muted mb-1">Fecha del Pesaje</label>
        <input
          id="weighingDate"
          type="date"
          value={date}
          ...
          className="w-full bg-c-surface-2 text-c-text p-3 rounded-xl text-lg disabled:opacity-70 disabled:bg-c-surface-2"
          required
        />
      </div>
      ...
      <div className="flex justify-end gap-3 pt-4">
        <button type="button" onClick={onCancel} className="px-5 py-2 bg-c-surface-2 hover:bg-c-surface-2 font-semibold rounded-lg text-c-text">
          Cancelar
        </button>
        <button
          type="submit"
          disabled={isLoading}
          className="px-5 py-2 bg-c-accent hover:bg-c-accent text-white font-bold rounded-lg disabled:opacity-50 flex items-center gap-2" // Añadido flex y gap
        >
