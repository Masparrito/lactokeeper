...
const FieldSet = ({ legend, children }: { legend: string, children: React.ReactNode }) => (
    <fieldset className="bg-c-surface-2/20 rounded-2xl p-4 border border-c-border-strong space-y-4">
        <legend className="px-2 text-sm font-semibold text-c-text-muted">{legend}</legend>
        {children}
    </fieldset>
);
            ...
            <form onSubmit={handleSaveActivity} className="space-y-6">
                <FieldSet legend="Paso 1: Define la Actividad">
                    <div className="grid grid-cols-2 gap-4">
                        <button type="button" onClick={() => setCategory('Tratamiento')} className={`p-4 rounded-2xl border-2 transition-colors flex flex-col items-center gap-2 ${category === 'Tratamiento' ? 'bg-blue-500/20 border-blue-500' : 'bg-c-surface-2 border-c-border hover:border-c-border-strong'}`}><Syringe className="w-8 h-8 text-blue-400" /><span className="font-semibold text-c-text-strong">Tratamiento</span></button>
                        <button type="button" onClick={() => setCategory('Control')} className={`p-4 rounded-2xl border-2 transition-colors flex flex-col items-center gap-2 ${category === 'Control' ? 'bg-purple-500/20 border-purple-500' : 'bg-c-surface-2 border-c-border hover:border-c-border-strong'}`}><ClipboardCheck className="w-8 h-8 text-purple-400" /><span className="font-semibold text-c-text-strong">Control</span></button>
                    </div>
                     <input list="activity-names" value={name} onChange={e => setName(e.target.value)} placeholder={`Nombre del ${category}...`} className="w-full bg-c-surface-2 text-c-text p-3 rounded-xl" required />
                ...
                {category === 'Tratamiento' && (
                    <FieldSet legend="Paso 2: Selecciona los Productos">
                        <div className="flex items-center gap-2">
                             <select value={productId || ''} onChange={e => setProductId(e.target.value)} className="w-full bg-c-surface-2 text-c-text p-3 rounded-xl" required><option value="">Producto Principal...</option>{products.map(p => <option key={p.id} value={p.id!}>{p.name}</option>)}</select>
                            <button type="button" onClick={() => setIsProductModalOpen(true)} className="flex-shrink-0 p-3 bg-teal-600 text-white rounded-xl"><PlusCircle size={20} /></button>
                        </div>
                        <select value={complementaryProductId || ''} onChange={e => setComplementaryProductId(e.target.value)} className="w-full bg-c-surface-2 text-c-text p-3 rounded-xl"><option value="">Producto Complementario (Opcional)...</option>{products.map(p => <option key={p.id} value={p.id!}>{p.name}</option>)}</select>
                    </FieldSet>
                )}
                ...
                <FieldSet legend={`Paso ${category === 'Tratamiento' ? '3' : '2'}: ¿Cuándo se ejecuta?`}>
                    {targetGroup === 'Maternidad' ? (
                        <div className="animate-fade-in">
                            <label className="block text-xs font-medium text-c-text-muted mb-2">A los Días de Nacido</label>
                            <div className="flex flex-wrap gap-2">{MATERNITY_DAYS.map(day => (<button type="button" key={day} onClick={() => setTriggerDays(String(day))} className={`px-3 py-1 text-sm font-semibold rounded-full transition-colors ${triggerDays === String(day) ? 'bg-teal-500 text-white' : 'bg-c-surface-2 text-c-text-strong'}`}>{day} días</button>))}</div>
                            <div className="flex items-center gap-2 mt-4"><input type="number" value={triggerDays} onChange={e => setTriggerDays(e.target.value)} placeholder="O escribe los días..." className="w-full bg-c-surface-2 text-c-text p-2 rounded-lg" required /></div>
                        </div>
                    ) : (
                        <>
                            <select value={triggerType} onChange={e => setTriggerType(e.target.value as any)} className="w-full bg-c-surface-2 text-c-text p-2 rounded-lg"><option value="fixed_date_period">Por Fecha Fija Anual</option><option value="birthing_season_event">Relativo a Temporada de Partos</option></select>
                            {triggerType === 'fixed_date_period' && (<div className="grid grid-cols-2 gap-4 animate-fade-in"><div><label htmlFor="triggerMonth" className="block text-xs font-medium text-c-text-muted mb-1">Mes</label><select id="triggerMonth" value={triggerMonth} onChange={e => setTriggerMonth(e.target.value)} className="w-full bg-c-surface-2 text-c-text p-2 rounded-lg">{months.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}</select></div><div><label htmlFor="triggerWeek" className="block text-xs font-medium text-c-text-muted mb-1">Semana</label><select id="triggerWeek" value={triggerWeek} onChange={e => setTriggerWeek(e.target.value)} className="w-full bg-c-surface-2 text-c-text p-2 rounded-lg">{weeks.map(w => <option key={w.value} value={w.value}>{w.label}</option>)}</select></div></div>)}
                            {triggerType === 'birthing_season_event' && (<div className="animate-fade-in"><label htmlFor="triggerOffsetDays" className="block text-xs font-medium text-c-text-muted mb-1">Días antes (-) / después (+) del inicio de la temporada</label><input id="triggerOffsetDays" type="number" value={triggerOffsetDays} onChange={e => setTriggerOffsetDays(e.target.value)} placeholder="Ej: -30 (para 30 días antes)" className="w-full bg-c-surface-2 text-c-text p-2 rounded-lg" required /></div>)}
                        </>
                    )}
                </FieldSet>
