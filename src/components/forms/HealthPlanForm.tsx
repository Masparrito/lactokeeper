...
import { HealthPlan, FemaleLifecycleStage, MaleLifecycleStage } from '../../db/local';
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-4">
        <div>
          ...
          <input id="name" type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Ej: Plan Sanitario Cabritonas" className="w-full bg-zinc-800 p-3 rounded-xl" required />
        </div>
        <div>
          <label htmlFor="description" className="block text-sm font-medium text-zinc-400 mb-1">Descripción (Opcional)</label>
          <input id="description" type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="Ej: Protocolo para animales de 3 a 7 meses" className="w-full bg-zinc-800 p-3 rounded-xl" />
        </div>
      </div>
