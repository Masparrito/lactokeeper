    ...
    <form onSubmit={handleSubmit} className="flex flex-col h-full">
            ...
            return (
              <div key={animal.id} className="grid grid-cols-2 items-center gap-4 p-2 bg-black/20 rounded-lg">
                <div className="min-w-0 col-span-1">
                    <label htmlFor={`weight-${animal.id}`} className="font-mono font-semibold text-base text-white truncate block cursor-pointer">{animal.id.toUpperCase()}</label>
                    {formattedName && (
                ...
                <input 
                  ...
                  // --- CAMBIO: Tipo explícito para 'e' ---
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleWeightChange(animal.id, e.target.value)}
                  placeholder="Kg"
      ...
      <div className="flex-shrink-0 p-4 border-t border-brand-border bg-ios-modal-bg">
        ...
        <div className="flex justify-end gap-3">
            ...
            <button 
                type="submit" 
                // --- CAMBIO: Lógica de disabled corregida ---
